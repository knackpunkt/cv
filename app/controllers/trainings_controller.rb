class TrainingsController < ApplicationController
  ...
  def index
     if(params.has_key?(:job_type))
      @trainings = Training.where(job_type: params[:job_type]).order("created_at asc")
    else
      @trainings = Training.all.order("created_at asc")
    end
  end
  ...
  private
    ...
    def training_params
      params.require(:training).permit(:frage, :antwort, :job_type)
    end
end
